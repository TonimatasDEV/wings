...
import (
	...
	"github.com/tonimatasdev/wings/events"
)
...
// ProcessEnvironment Defines the basic interface that all environments need to implement so that
// a server can be properly controlled.
type ProcessEnvironment interface {
	// Type Returns the name of the environment.
	Type() string

	// Config Returns the environment configuration to the caller.
	Config() *Configuration

	// Events Returns an event emitter instance that can be hooked into to listen for different
	// events that are fired by the environment. This should not allow someone to publish
	// events, only subscribe to them.
	Events() *events.Bus

	// Exists Determines if the server instance exists. For example, in a docker environment
	...
	// InSituUpdate Performs an update of server resource limits without actually stopping the server
	// process. This only executes if the environment supports it, otherwise it is
	// a no-op.
	InSituUpdate() error

	// OnBeforeStart Runs before the environment is started. If an error is returned starting will
	// not occur, otherwise proceeds as normal.
	OnBeforeStart(ctx context.Context) error

	// Start Starts a server instance. If the server instance is not in a state where it
	...
	// Destroy Destroys the environment removing any containers that were created (in Docker
	// environments at least).
	Destroy() error

	// ExitState Returns the exit state of the process. The first result is the exit code, the second
	// determines if the process was killed by the system OOM killer.
	ExitState() (uint32, bool, error)

	// Create Creates the necessary environment for running the server process. For example,
	...
	// SendCommand Sends the provided command to the running server instance.
	SendCommand(string) error

	// Readlog Reads the log file for the process from the end backwards until the provided
	...
	// SetState Sets the current state of the environment. In general you should let the environment
